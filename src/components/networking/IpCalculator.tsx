...
function ResultBox({ label, value, color, helpText }: { label: string, value: string, color: string, helpText?: string }) {
  return (
    <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-800 relative group transition-colors hover:border-slate-700">
      <div className="text-xs text-slate-400 uppercase font-bold flex justify-between items-center mb-1">
        {label}
...
export default function IpCalculator() {
  ...
  const renderBinaryMask = (currentCidr: number) => {
    ...
    for (let i = 0; i < 32; i++) {
      ...
      if ((i + 1) % 8 === 0 && i !== 31) {
        bits.push(<span key={`dot-${i}`} className="text-slate-400 font-bold mx-1">.</span>);
      }
    }
    return bits;
  };
    ...
    <div className="w-full max-w-5xl mx-auto p-4 sm:p-6 bg-[#181926] text-slate-50 rounded-xl shadow-2xl border border-slate-800 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-wrap gap-6 items-end bg-slate-900/30 p-5 rounded-lg border border-slate-800/50">
        <div className="flex flex-col gap-2 flex-1 min-w-[200px]">
          <label className="text-xs text-slate-400 uppercase font-bold tracking-wider">Endereço IP</label>
          <input 
            type="text" 
            value={ip}
            onChange={(e) => setIp(e.target.value)}
            className="bg-[#181926] text-emerald-400 border border-slate-700 p-3 rounded-md focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 font-mono transition-all"
            placeholder="Ex: 192.168.1.0"
          />
        </div>
        <div className="flex flex-col gap-2 flex-1 min-w-[300px]">
          <label className="text-xs text-slate-400 uppercase font-bold tracking-wider">Prefixo CIDR (/{cidr})</label>
          <div className="flex items-center gap-4 bg-[#181926] border border-slate-700 p-2 rounded-md">
            <input 
      ...
      <div className="flex gap-2 flex-wrap items-center">
        <span className="text-xs text-slate-400 uppercase font-bold mr-2">Comuns:</span>
        {quickCidrs.map(c => (
          <button 
      ...
      {!details ? (
        ...
        <>
          ...
          <div className="bg-slate-900/30 p-5 rounded-lg border border-slate-800/50 mt-6">
            <div className="text-xs text-slate-400 uppercase font-bold mb-3">Espectro / Range da sub-rede:</div>
          ...
          {/* BINÁRIO */}
          <div className="mt-6">
            <div className="text-xs text-slate-400 uppercase font-bold mb-2">Máscara em binário (32 bits):</div>
