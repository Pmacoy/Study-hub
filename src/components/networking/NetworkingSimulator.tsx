    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        <button onClick={() => setView('packets')}
          className={`px-4 py-2 rounded-2xl text-sm font-semibold transition-all ${view === 'packets' ? 'bg-emerald-500/20 border border-emerald-500/40 text-emerald-300' : 'border border-slate-800 text-slate-400 hover:text-slate-300'}`}>
          📦 Anatomia do Pacote
        </button>
        <button onClick={() => setView('handshake')}
          className={`px-4 py-2 rounded-2xl text-sm font-semibold transition-all ${view === 'handshake' ? 'bg-sky-500/20 border border-sky-500/40 text-sky-300' : 'border border-slate-800 text-slate-400 hover:text-slate-300'}`}>
          🤝 TCP Handshake
        </button>
      </div>
