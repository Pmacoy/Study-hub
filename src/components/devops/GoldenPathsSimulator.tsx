...
import { Copy, Check, Route, Zap, ShieldOff, Rocket, FileText, LogOut } from 'lucide-react';
...
function InteractiveScaffolder() {
  ...
  return (
    <div className="rounded-2xl border border-violet-500/20 bg-[#181926]/50 p-4 space-y-4">
      <div className="text-xs font-black text-violet-400 uppercase tracking-widest">🎬 Scaffolder — cria um serviço</div>

      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-2xs text-slate-400 uppercase font-bold">Nome do serviço</label>
          <input value={name} onChange={e => setName(e.target.value)}
            className="mt-1 w-full px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-sm text-white font-mono focus:border-violet-500/40 focus:outline-none" />
        </div>
        <div>
          <label className="text-2xs text-slate-400 uppercase font-bold">Tier de reliability</label>
          <div className="mt-1 flex gap-1">
            {(['gold', 'silver', 'bronze'] as const).map(t => (
              <button key={t} onClick={() => setTier(t)}
                className={`flex-1 py-1.5 rounded-xl text-xs font-bold transition-all ${tier === t ? 'bg-violet-500/25 text-violet-200 border border-violet-500/40' : 'bg-slate-900 text-slate-400 border border-slate-800 hover:text-slate-300'}`}>
                {t === 'gold' ? '🥇' : t === 'silver' ? '🥈' : '🥉'} {t}
              </button>
            ))}
          </div>
        ...
        <div>
          <label className="text-2xs text-slate-400 uppercase font-bold">Linguagem</label>
          <div className="mt-1 flex gap-1">
            {(['typescript', 'python', 'go'] as const).map(l => (
              <button key={l} onClick={() => setLang(l)}
                className={`flex-1 py-1.5 rounded-xl text-2xs font-bold transition-all ${lang === l ? 'bg-violet-500/25 text-violet-200 border border-violet-500/40' : 'bg-slate-900 text-slate-400 border border-slate-800 hover:text-slate-300'}`}>
                {l}
              </button>
            ))}
          </div>
        ...
        <div>
          <label className="text-2xs text-slate-400 uppercase font-bold">Base de dados?</label>
          <div className="mt-1">
            <button onClick={() => setDb(!db)}
              className={`w-full py-1.5 rounded-xl text-xs font-bold transition-all ${db ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : 'bg-slate-900 text-slate-400 border border-slate-800'}`}>
      ...
      <div className="pt-3 border-t border-slate-800">
        <div className="text-2xs font-black text-emerald-400 uppercase tracking-widest mb-2">O que o platform team cria por ti (em ~90s)</div>
...
export default function GoldenPathsModule() {
  ...
  const views = [
    { id: 'what' as const, label: <><Route size={14} className="mr-1 inline" /> O que são Golden Paths?</> },
    { id: 'paved' as const, label: <><Rocket size={14} className="mr-1 inline" /> Paved Road (demo)</> },
    { id: 'template' as const, label: <><FileText size={14} className="mr-1 inline" /> Template real</> },
    { id: 'escape' as const, label: <><LogOut size={14} className="mr-1 inline" /> Escape hatches</> },
  ];
        ...
        <div className="space-y-4">
          <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-lg font-bold text-violet-300">Golden Path</div>
          ...
          <div className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-sm text-slate-400 italic">
          ...
          <div>
            <div className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Golden Paths típicos</div>
            <div className="grid grid-cols-1 gap-2">
              {[
                ...
                <div key={g.k} className="flex items-start gap-3 p-3 rounded-xl bg-[#181926] border border-slate-800">
                  <div className="w-8 h-8 rounded-lg bg-violet-500/15 text-violet-400 flex items-center justify-center shrink-0">
                    {g.i}
                  </div>
                  <div>
                    <div className="text-sm font-bold text-white">{g.k}</div>
                    <div className="text-2xs text-slate-400 mt-0.5">{g.v}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-4">
          <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-lg font-bold text-amber-300">Escape Hatch</div>
                <p className="text-sm text-slate-400 mt-1 leading-relaxed">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={g.title} className={`p-4 rounded-2xl border ${g.color === 'emerald' ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-amber-500/20 bg-amber-500/5'}`}>
                <div className={`text-xs font-black uppercase tracking-widest mb-2 ${g.color === 'emerald' ? 'text-emerald-400' : 'text-amber-400'}`}>{g.title}</div>
                {g.items.map((i, idx) => (
                  <div key={idx} className={`text-xs py-1 ${g.color === 'emerald' ? 'text-emerald-300/70' : 'text-amber-300/70'}`}>
                    · {i}
                  </div>
                ))}
              </div>
