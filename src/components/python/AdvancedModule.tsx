...
import { Copy, Check, Zap, Package, RefreshCw, Tag } from 'lucide-react';
...
export default function AdvancedModule() {
  ...
  const views = [
    { id: 'generators' as const, label: <><Zap size={14} className="mr-1 inline" /> Generators & Iterators</> },
    { id: 'modules' as const, label: <><Package size={14} className="mr-1 inline" /> Módulos & Packages</> },
    { id: 'async' as const, label: <><RefreshCw size={14} className="mr-1 inline" /> async/await</> },
    { id: 'typing' as const, label: <><Tag size={14} className="mr-1 inline" /> Type Hints</> },
  ];
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-xl border border-rose-500/20 bg-rose-500/5">
              <div className="text-2xs font-black text-rose-400 uppercase mb-1">Lista (Memória toda)</div>
              <code className="text-xs font-mono text-slate-300">squares_list(10_000_000)</code>
              <div className="text-xs text-rose-300 mt-1 font-bold">≈ 80 MB de RAM</div>
            </div>
            <div className="p-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5">
              <div className="text-2xs font-black text-emerald-400 uppercase mb-1">Generator (Lazy)</div>
              <code className="text-xs font-mono text-slate-300">squares_gen(10_000_000)</code>
              <div className="text-xs text-emerald-300 mt-1 font-bold">≈ 200 bytes de RAM</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={g.t} className={`p-4 rounded-2xl border ${g.c === 'emerald' ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-sky-500/20 bg-sky-500/5'}`}>
                <div className={`text-xs font-black uppercase tracking-widest mb-2 ${g.c === 'emerald' ? 'text-emerald-400' : 'text-sky-400'}`}>{g.t}</div>
                {g.items.map(i => <div key={i} className="flex items-start gap-1.5 text-xs text-slate-400 py-0.5"><span className={`${g.c === 'emerald' ? 'text-emerald-400' : 'text-sky-400'} shrink-0`}>·</span>{i}</div>)}
              </div>
            ))}
          </div>
        </div>
