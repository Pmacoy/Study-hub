  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <button onClick={onExit} className="flex items-center gap-2 text-[12px] text-slate-400 hover:text-slate-300">
          <ArrowLeft size={14} /> Sair da sessão
        </button>
        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{session.shell} · {session.difficulty}</div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-4">
        ...
        >
          <div className="flex items-center gap-2 px-4 py-2 bg-[#181926] border-b border-slate-800">
            <div className="flex gap-1.5">
              <div className="w-3 h-3 rounded-full bg-rose-500/60" />
              <div className="w-3 h-3 rounded-full bg-amber-500/60" />
              <div className="w-3 h-3 rounded-full bg-emerald-500/60" />
            </div>
            <span className="text-[10px] text-slate-400 font-mono ml-2">{session.shell} — Study Hub</span>
          </div>
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 font-mono text-[12px] leading-relaxed min-h-[420px] max-h-[520px]">
              ...
              <div key={i} className="mb-3">
                {entry.cmd && (
                  <div className="text-emerald-400">
                    <span className="text-slate-400">{session.prompt}</span>{entry.cmd}
                  </div>
                )}
                {entry.output && (
            ...
            <div className="flex items-center text-emerald-400">
              <span className="text-slate-400">{session.prompt}</span>
              <input
                ref={inputRef}
                value={input}
        ...
        <aside className="space-y-3">
          <div className="rounded-2xl border border-slate-800 bg-[#181926]/70 p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Objectivos</div>
              <div className="text-[10px] text-slate-400">{goalsHit.size}/{session.objectives.length}</div>
            </div>
                ...
                return (
                  <div key={o.id} className={`flex items-start gap-2 p-2 rounded-lg transition-all ${done ? 'bg-emerald-500/8' : 'bg-slate-900'}`}>
                    ...
                    <div className="min-w-0">
                      ...
                      {!done && <div className="text-[10px] text-slate-400 mt-0.5">{o.hint}</div>}
                    </div>
                  </div>
                );
              })}
